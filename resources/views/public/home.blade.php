...
@section('title', __('site.nav.home') . ' — ' . __('site.meta.default_title'))
...
@section('content')
    ...
    <section class="bg-navy-dark text-white">
        <div class="max-w-7xl mx-auto px-4 py-16 md:py-24">
            <div class="max-w-3xl">
                <p class="text-gold-light text-sm font-medium tracking-wider uppercase mb-3">{{ __('site.home.eyebrow') }}</p>
        ...
        <div class="max-w-7xl mx-auto px-4 py-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                <div>
                    ...
                    <p class="text-sm text-gray-500">{{ __('site.home.stats_articles') }}</p>
                </div>
                <div>
                    <p class="text-3xl font-bold text-navy">{{ $stats['journals'] }}</p>
                    <p class="text-sm text-gray-500">{{ __('site.home.stats_journals') }}</p>
                </div>
                <div>
                    <p class="text-3xl font-bold text-navy">{{ $stats['authors'] }}+</p>
                    <p class="text-sm text-gray-500">{{ __('site.home.stats_authors') }}</p>
                </div>
                <div>
                    <p class="text-3xl font-bold text-navy">{{ number_format($stats['citations']) }}</p>
                    <p class="text-sm text-gray-500">{{ __('site.home.stats_views') }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-4 py-12">
        <div class="flex items-center justify-between mb-8">
            <h2 class="font-display text-2xl font-bold text-navy-dark">{{ __('site.home.latest_articles') }}</h2>
            <a href="{{ route('articles.index') }}" class="text-sm text-gold hover:text-gold-light font-medium">{{ __('site.home.view_all') }}
                →</a>
        </div>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('articles.show', $article->slug) }}"
                            class="text-xs text-navy font-medium hover:text-gold">{{ __('site.home.read') }} →</a>
    ...
    @if($conferences->count() > 0)
        <section class="bg-navy-dark text-white">
            <div class="max-w-7xl mx-auto px-4 py-12">
                <h2 class="font-display text-2xl font-bold mb-8 text-gold-light">📅 {{ __('site.home.conferences_heading') }}</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach($conferences as $conf)
                        <div class="bg-navy rounded-xl p-6 border border-navy-light hover:border-gold transition-colors">
                            ...
                            <p class="text-sm text-blue-200 mb-4">📍 {{ $conf->venue ?? ($conf->is_online ? __('site.conferences.online') : '') }}</p>
                            <a href="{{ route('conferences.show', $conf->slug) }}"
                                class="text-sm text-gold hover:text-gold-light">{{ __('site.home.details') }} →</a>
                        </div>
                    @endforeach
                </div>
            </div>
    ...
    {{-- Journals --}}
    <section class="max-w-7xl mx-auto px-4 py-12">
        <h2 class="font-display text-2xl font-bold text-navy-dark mb-8">📰 {{ __('site.home.journals_heading') }}</h2>
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-shadow flex gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <a href="{{ route('journals.show', $journal->slug) }}"
                            class="text-xs text-gold font-medium hover:text-gold-light mt-2 inline-block">{{ __('site.home.view') }} →</a>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    @if($news->count() > 0)
        <section class="bg-gray-50 border-t">
            <div class="max-w-7xl mx-auto px-4 py-12">
                <div class="flex items-center justify-between mb-8">
                    <h2 class="font-display text-2xl font-bold text-navy-dark">📢 {{ __('site.home.news_heading') }}</h2>
                    <a href="{{ route('news.index') }}" class="text-sm text-gold hover:text-gold-light font-medium">{{ __('site.home.view_all') }}
                        →</a>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <section class="bg-gold-pale border-t">
        <div class="max-w-7xl mx-auto px-4 py-12 text-center">
            <h2 class="font-display text-2xl font-bold text-navy-dark mb-2">{{ __('site.home.newsletter_heading') }}</h2>
            <p class="text-gray-600 mb-6">{{ __('site.home.newsletter_text') }}
            </p>
            <form method="POST" action="{{ route('subscribe') }}" class="flex max-w-md mx-auto gap-2">
                @csrf
                <input type="email" name="email" placeholder="{{ __('site.home.email_placeholder') }}" required
                    class="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-navy outline-none">
                <button
                    class="bg-navy hover:bg-navy-dark text-white font-medium px-5 py-2.5 rounded-lg transition-colors text-sm">{{ __('site.home.subscribe') }}</button>
            </form>
