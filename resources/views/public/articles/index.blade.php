...
@section('title', __('site.articles.title') . ' — ' . __('site.footer.brand'))
@section('content')
    <div class="max-w-7xl mx-auto px-4 py-12">
        <h1 class="font-display text-3xl font-bold text-navy-dark mb-6">{{ __('site.articles.title') }}</h1>
        {{-- Filters --}}
        <div class="bg-white rounded-xl border p-5 mb-6">
            <form method="GET" class="flex flex-wrap gap-3">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('site.articles.search_placeholder') }}"
                ...
                <select name="journal_id" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    <option value="">{{ __('site.articles.all_journals') }}</option>
                ...
                <select name="year" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    <option value="">{{ __('site.articles.all_years') }}</option>
                    @foreach($years as $y)
                        <option value="{{ $y }}" {{ request('year') == $y ? 'selected' : '' }}>{{ $y }}</option>
                    @endforeach
                </select>
                <select name="language" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
                    <option value="">{{ __('site.articles.all_languages') }}</option>
                    @foreach(['uz' => 'O\'zbek', 'en' => 'English', 'ru' => 'Русский'] as $v => $l)
                        <option value="{{ $v }}" {{ request('language') === $v ? 'selected' : '' }}>{{ $l }}</option>
                    @endforeach
                </select>
                <button class="bg-navy text-white text-sm px-4 py-2 rounded-lg">{{ __('site.articles.search') }}</button>
            </form>
        </div>
        ...
        <div class="space-y-4">
                    ...
                    <div class="flex items-start gap-4">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <span class="text-xs text-gray-400">{{ __('site.journal.volume') }} {{ $article->volume }}, {{ __('site.journal.issue') }} {{ $article->issue }}</span>
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                @if($article->pdf_file)
                                    ...
                                    <a href="{{ route('articles.pdf', $article->slug) }}"
                                        ...
                                        {{ __('site.articles.pdf') }}</a>
                                @endif
                                <a href="{{ route('articles.show', $article->slug) }}"
                                    class="text-xs text-navy font-medium hover:text-gold">{{ __('site.articles.abstract_link') }} →</a>
            ...
            @empty
                <div class="text-center py-12 text-gray-400">{{ __('site.articles.not_found') }}</div>
            @endforelse
        </div>
